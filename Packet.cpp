...
// using buffer, split read and write
ssize_t readn(int fd, std::string& inbuffer, bool& zero){
	ssize_t nread;
	ssize_t readSum = 0;
	while(true){
		char buff[MAXLINE];
		if( (nread = read(fd, buff, MAXLINE)) < 0){// if error
			if(errno == EINTR)
				continue;
...
ssize_t writen(int fd, const void* vptr, size_t n){
	...
	while(nleft > 0){
		if( (nwritten = write(fd, ptr, nleft)) <= 0){
			if(nwritten < 0 && errno == EINTR){//interupt error
				nwritten = 0;
			}
			else{// < 0 or = 0
				return -1;
		...
		nleft -= nwritten;	// prevent nwritten cannot = 0 in normal case
		ptr += nwritten;	// record how many bits in buffer
	}
	return n;
}
